...
main() {
  local theme
  theme="$(get_tmux_option "@tokyonight_flavour" "night")"
  ...
  # variables.
  # shellcheck source=tokyonight-night.tmuxtheme
  source /dev/stdin <<<"$(sed -e "/^[^#].*=/s/^/local /" "${PLUGIN_DIR}/tokyonight-${theme}.tmuxtheme")"
  ...
  # NOTE: Checking for the value of @tokyonight_window_tabs_enabled
  local wt_enabled
  ...
  readonly show_directory_in_window_status_current="#[fg=$thm_black,bg=$thm_orange] #I#F #W #[fg=$thm_black,bg=$thm_magenta] #(echo '#{pane_current_path}' | rev | cut -d'/' -f-2 | rev) "
 ...
 #setw -g window-status-current-format "#[fg=colour232,bg=$thm_orange] #I #[fg=colour255,bg=colour237] #(echo '#{pane_current_path}' | rev | cut -d'/' -f-2 | rev) "
  ...
  # NOTE: With the @tokyonight_window_tabs_enabled set to on, we're going to
